...
class QueueHandler(logging.Handler):
    """Custom logging handler to emit logs to a queue."""
...
def clean_text(text):
    """
    Perform basic cleaning of the text by removing extra whitespace and non-ASCII characters.
    """
...
def format_as_markdown(text, title, file_path):
    """
...
def process_file(file_path, output_dir, single_file):
    """
    Process a single text file: clean, format, and either save to an individual file or return content.
    """
    ...
    try:
        ...
        markdown_text = format_as_markdown(cleaned_text, title, file_path)
...
def process_folder(folder_path, output_dir, single_file, combined_content, all_files):
    ...
    for root, dirs, files in os.walk(folder_path):
        # Exclude folders that should be skipped
        dirs[:] = [d for d in dirs if d not in SKIP_FOLDERS]
        for file in files:
            file_path = os.path.join(root, file)
            if any(file.endswith(ext) for ext in TEXT_EXTENSIONS):
                all_files.append(file_path)
                result = process_file(file_path, output_dir, single_file)
                if result:
...
class AutoMDApp:
    """Main application class for the AutoMD GUI."""
    ...
    def save_config(self):
        ...
        self.config['DEFAULT']['SingleFile'] = 'True'
    ...
    def create_input_widgets(self, frame):
        ttk.Label(frame, text="Input Files or GitHub Repos:").grid(row=0, column=0, sticky=tk.W, pady=5)
        self.input_files_entry = scrolledtext.ScrolledText(frame, width=70, height=5)
        ...
        ttk.Button(frame, text="Browse", command=self.browse_input_files).grid(row=0, column=2, pady=5)

    def create_output_widgets(self, frame):
        ttk.Label(frame, text="Output:").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.output_entry = ttk.Entry(frame, width=70)  # Width adjusted to fill the space
        self.output_entry.grid(row=1, column=1, pady=5, padx=(0, 5), sticky=(tk.W, tk.E))
        self.output_entry.insert(0, self.config['DEFAULT']['OutputPath'])
        ttk.Button(frame, text="Browse", command=self.browse_output).grid(row=1, column=2, pady=5, sticky=tk.W)

    def create_repo_depth_widgets(self, frame):
        ttk.Label(frame, text="Repository clone depth:").grid(row=2, column=0, sticky=tk.W, pady=5)
        self.repo_depth_var = tk.StringVar(value=self.config['DEFAULT']['RepoDepth'])
        repo_depth_combo = ttk.Combobox(
            frame,
            textvariable=self.repo_depth_var,
            values=["Full", "1", "5", "10", "20", "50", "100"]
        )
        repo_depth_combo.grid(row=2, column=1, sticky=tk.W, pady=5)

    def create_misc_widgets(self, frame):
        self.open_folder_var = tk.BooleanVar(value=self.config['DEFAULT'].getboolean('OpenFolder'))
        ttk.Checkbutton(
            frame,
            text="Open containing folder after processing",
        ...
        self.console_output = scrolledtext.ScrolledText(frame, height=25, width=70)  # Height increased
        self.console_output.grid(row=6, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=10)
        self.console_output.config(state=tk.DISABLED)

        ttk.Button(frame, text="Start Processing", command=self.start_processing).grid(row=7, column=1, pady=10)
    ...
    def start_processing(self):
        ...
        single_file = True  # Always output to a single file
        ...
        def process_thread():
            try:
                output_dir = process_input(
                    input_files, output_path, self.temp_dir, single_file, repo_depth
                )
